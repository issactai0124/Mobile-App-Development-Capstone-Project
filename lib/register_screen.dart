...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  List<String> _countries = fallbackCountries;
  ...
  Future<void> _loadCountries() async {
    try {
      ...
      // Ensure 'United States' or current selection is in the list
      if (!countries.contains(_country)) {
        countries = [_country, ...countries];
        // Or if we just want a unified list
      }
      // Sort alphabetically if desired, but API returns unsorted usually
      countries.sort();

      if (countries.isNotEmpty) {
        setState(() {
          _countries = countries;
          // Security check: if somehow _country is not in list (even after adding?), reset
      ...
      // Use fallback list which is already set. Just notify user.
      _showToast('Error fetching countries. Using offline list.');
